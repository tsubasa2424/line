...
@app.route('/')
def index():
    current_prices = {symbol: get_price(symbol) for symbol in CURRENCY_NAMES.keys()}  # 各通貨の現在価格を取得
    return render_template("index.html", currencies=CURRENCY_NAMES, watchlist=watchlist, current_prices=current_prices)
...
def add_currency():
    ...
    # 通貨を追加した後に、わかりやすいメッセージを表示
    watchlist.append({"symbol": symbol, "price": target_price, "condition": condition})
    return render_template("index.html", currencies=CURRENCY_NAMES, watchlist=watchlist, current_prices={symbol: current_price}, message="通貨を追加しました！")
