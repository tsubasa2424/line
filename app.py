...
def index():
    ...
    if request.method == 'POST':
        ...
        target_prices = request.form.getlist('price')
        conditions = request.form.getlist('condition')

        # 各通貨の情報を監視リストに追加
        for symbol, target_price, condition in zip(symbols, target_prices, conditions):
            current_price = get_price(symbol)
            if current_price is None:
                return "価格を取得できませんでした", 400

            watchlist.append({"symbol": symbol, "price": float(target_price), "condition": condition})
