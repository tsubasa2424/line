...
# LINE通知関数（デバッグ用にレスポンス表示）
def send_line_notification(message):
    url = "https://notify-api.line.me/api/notify"
    headers = {"Authorization": f"Bearer {LINE_NOTIFY_TOKEN}"}
    data = {"message": message}
    response = requests.post(url, headers=headers, data=data)
    print(f"LINE通知送信: {message} | レスポンス: {response.status_code}, {response.text}")
...
def monitor_prices():
    ...
    while True:
        try:
            print("監視中... 現在のウォッチリスト:", watchlist)
            new_watchlist = []
            for item in watchlist:
                symbol, target_price, condition = item["symbol"], item["price"], item["condition"]
                current_price = get_price(symbol)

                print(f"現在価格 {symbol}: {current_price} 円 (目標: {target_price} 円 {condition})")
...
@app.route('/')
def index():
    return render_template('index.html', currencies=CURRENCY_NAMES)
